...
# TODO: add more numbers conversions!
...
(1..1000).each do |num|
  $word_of_numbers << number_to_words(num)
end
puts $word_of_numbers
puts "#{$word_of_numbers.join('').gsub(/\s+/, '').length} found in #{Time.now - start_time} seconds"
